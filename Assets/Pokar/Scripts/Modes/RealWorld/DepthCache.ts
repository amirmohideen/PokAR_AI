...
export class DepthCache extends BaseScriptComponent {
  @input @allowUndefined camModule: CameraModule;
  ...
  saveDepthFrame(): number {
    ...
    if (this.latestCameraDepthPair == null) {
      print("[DepthCache] Warning: No depth/camera frames available yet");
      return -1;
    }
  ...
  getCamImageWithID(depthFrameID: number): Texture | null {
    let cached = this.cachedDepthFrames.get(depthFrameID);
    if (cached == null || cached.colorCameraFrame == null) {
      print("[DepthCache] Warning: No cached frame for ID: " + depthFrameID);
      return null;
    }
    return cached.colorCameraFrame.imageFrame;
  }
