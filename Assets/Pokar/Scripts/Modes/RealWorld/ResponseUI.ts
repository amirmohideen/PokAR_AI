...
export class ResponseUI extends BaseScriptComponent {
  @input @allowUndefined responseAIText: Text;
  @input @allowUndefined worldLabelPrefab: ObjectPrefab;
  @input @allowUndefined worldArrowPrefab: ObjectPrefab;
  @input @allowUndefined responseUIObj: SceneObject;
  ...
  loadWorldLabel(label: string, worldPosition: vec3, useArrow: boolean) {
    ...
    // Disable the LookAtComponent
    var lookAt = labelObj.getComponent("LookAtComponent");
    if (lookAt) {
